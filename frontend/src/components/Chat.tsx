...
/* Let TS know MathJax may exist on window */
declare global {
  interface Window {
...
function render(text: string): string {
  const html = marked.parse(text) as string; // marked types are union; force sync path
...
export default function Chat({ messages }: ChatProps) {
  ...
  useEffect(() => {
    // re-typeset math when messages change (safe if MathJax not present)
