...
class TextGenerationPipeline {
  ...
  /** Call this when the user picks a model */
  static setModel(modelId: string) {
    if (!modelId || modelId === this.model_id) return; // no-op if same
    this.model_id = modelId;
    // Clear caches so the next getInstance() loads the new model
  ...
  static async getInstance(
    ...
    console.log("Loading: ", this.model_id);
...
async function load(modelId?: string): Promise<void> {
  ...
  if (modelId) {
    TextGenerationPipeline.setModel(modelId); // <-- switch model + clear caches
  }
